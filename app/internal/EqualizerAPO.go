...
var equalizerAPOLatestVerRe = regexp.MustCompile(`(\d\.\d\.\d)`)

func EqualizerAPO(getExec func() (string, error)) CheckResult {
	...
	latestVer, err := func() (string, error) {
		content, err := utils.Fetch("https://sourceforge.net/projects/equalizerapo/rss?path=/")
		if err != nil {
			return "", fmt.Errorf("can't fetch the homepage: %v", err)
		}
		match := equalizerAPOLatestVerRe.FindStringSubmatch(string(content))
		if match == nil {
	...
	return CheckResult{
		...
		DownloadPageURL: "https://sourceforge.net/projects/equalizerapo/files/latest/download",
	}
}
